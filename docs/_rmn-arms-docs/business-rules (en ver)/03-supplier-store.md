...
| User ID | Owner account reference |
| Business Name | Legal business name |
| Business Type | INDIVIDUAL, SOLE_PROPRIETORSHIP, LLC, CORPORATION, PARTNERSHIP |
...
| Business Registration Number | State/country registration ID |
| Registration Country | Country code (ISO 3166-1) |
| Registration State | State/province code |
| Incorporation Date | Business founding date |
...
| Billing Address | Billing address (may differ) |

#### Account Status

| Attribute | Values |
|-----------|--------|
| Status | PENDING_REGISTRATION, ACTIVE, INACTIVE, SUSPENDED, PERMANENTLY_BANNED |
| Tier | STARTER (1-5 devices), PROFESSIONAL (6-50), ENTERPRISE (51+) |
...
| Attribute | Description |
|-----------|-------------|
| Wallet | Linked wallet for payouts |
| Revenue Share | Default 80% (supplier's share) |
| Payment Schedule | WEEKLY (Monday), BIWEEKLY, MONTHLY (1st) |
| Minimum Payout | Threshold to trigger payout (default $50) |

#### Performance Metrics

| Metric | Description |
|--------|-------------|
| Total Stores | Count of registered stores |
| Total Devices | Count of active devices |
| Total Impressions | Lifetime impression count |
| Total Revenue | Lifetime revenue (USD) |
| Average Device Uptime | % uptime across all devices |
...
| Setting | Description |
|---------|-------------|
| Auto-Approve Campaigns | Auto-approve if passes blocking rules |
| Notification Preferences | Email, SMS, Push preferences |
...
| Attribute | Description |
|-----------|-------------|
| Verification Type | BUSINESS, TAX, BANK_ACCOUNT |
| Status | UNVERIFIED, PENDING, VERIFIED, REJECTED |
| Documents | Uploaded verification files |
| Business Documents | Registration, tax, proof of address, bank statement |
| Reviewed By | Admin who reviewed |
| Rejection Reason | If rejected, the reason |
...
|---------|-------------|
| Auto-Approve Campaigns | Auto-approve if passes blocking rules |
| Require Manual Review | For sensitive categories |
| Payment Schedule | WEEKLY, BIWEEKLY, MONTHLY |
| Minimum Payout | USD threshold |
| Payout Method | BANK_TRANSFER, PAYPAL, STRIPE |
| Require Device Approval | Admin approval for new devices |
| Max Devices Per Store | Limit per store location |
| Device Offline Alert | Minutes before alert sent |
| Low Revenue Alert | Daily revenue threshold |
...
| Attribute | Description |
|-----------|-------------|
| Store Name | Display name (e.g., "Whole Foods - Downtown Seattle") |
| Store Code | Internal supplier code (optional) |
| Store Type | GROCERY, RETAIL, RESTAURANT, GYM, PHARMACY, CONVENIENCE, DEPARTMENT_STORE, SHOPPING_MALL, OTHER |
...
| Timezone | IANA timezone |
| Geofence Radius | For location verification (default 100m) |

#### Store Profile

| Attribute | Description |
|-----------|-------------|
| Square Footage | Store size (sq ft) |
| Average Daily Visitors | Estimated foot traffic |
| Operating Hours | Daily opening/closing times |
| Primary Category | "Grocery", "Fashion", etc. |
| Subcategories | Additional tags like "Organic", "Premium" |

#### Device & Performance

| Metric | Description |
|--------|-------------|
| Max Devices Allowed | Based on store size and tier |
| Total Devices | Current device count |
| Active Devices | Currently online |
| Total Impressions | Impressions served |
| Total Revenue | Revenue generated |
| Average Uptime | % across devices |
| Quality Score | 0-100 |
...
| Value | Description |
|-------|-------------|
| ACTIVE | Operational, devices serve ads |
...
The system calculates and tracks supplier revenue daily based on verified impressions. Key metrics include:
- Total impressions per day
- Total revenue per day
- Average CPM achieved

**Revenue by Device**:
Suppliers can view revenue breakdown by each device, including:
- Impressions served per device
- Revenue generated per device
- Average revenue per impression
- Store name for each device
...
| Attribute | Description |
|-----------|-------------|
| Rule Name | Descriptive name (e.g., "Block Athletic Competitors") |
| Rule Type | ADVERTISER, INDUSTRY, KEYWORD, CUSTOM |
| Is Active | Whether rule is currently active |

#### Blocking Criteria

| Criteria | Description |
|----------|-------------|
| Blocked Advertisers | Specific advertiser IDs to block |
| Blocked Industries | Industry categories (e.g., "Athletic Footwear") |
| Blocked Keywords | Keywords in campaign/content (e.g., "Adidas", "Reebok") |
...
| Specific Stores | Rule applies only to listed stores |
