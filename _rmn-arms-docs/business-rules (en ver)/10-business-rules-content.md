...
interface ContentAsset {
  ...
  advertiser_id: string                  // FK to advertiser (owner)
  user_id: string                        // FK to user who uploaded
  ...
  // Storage
  storage_url: string                    // S3/GCS URL (original file)
  cdn_url: string                        // CloudFront/Cloudflare URL (cached)
  thumbnail_url: string | null           // Preview thumbnail (for videos/PDFs)

  // Media Properties (if applicable)
  width: number | null                   // Pixel width (images/videos)
  height: number | null                  // Pixel height (images/videos)
  aspect_ratio: string | null            // "16:9", "9:16", "1:1", etc.
  duration_seconds: number | null        // Duration (videos/audio)
  frame_rate: number | null              // FPS (videos)
  bitrate_kbps: number | null           // Bitrate (videos/audio)
  codec: string | null                   // "H.264", "VP9", etc.

  // Metadata
  title: string                          // Display name
  description: string | null             // Optional description
  tags: string[]                         // User-defined tags for search
  category: string | null                // "Food", "Fashion", "Electronics", etc.
  brand: string | null                   // Brand name (if applicable)

  // Lifecycle
  status: ContentStatus                  // UPLOADED | PROCESSING | PENDING_APPROVAL | APPROVED | REJECTED | ACTIVE | ARCHIVED
  uploaded_at: Date
  ...
  moderation_score: number | null        // AI moderation confidence score (0-100)
  moderation_flags: string[]             // ["adult_content", "violence", etc.]
  moderated_by_user_id: string | null    // Manual reviewer user ID
  moderation_notes: string | null

  // Usage & Performance
  used_in_campaigns_count: number        // # of campaigns using this asset
  total_impressions: number              // Total impressions across all campaigns
  total_clicks: number | null            // Clicks (if interactive content)
  average_ctr: number | null             // Click-through rate (%)

  // Licensing & Rights
  license_type: LicenseType              // OWNED | LICENSED | STOCK | USER_GENERATED
  license_expiry_date: Date | null       // Expiration date for licensed content
  rights_holder: string | null           // Who owns the content
  usage_rights_confirmed: boolean        // Advertiser confirmed they have rights

  // Organization
  folder_id: string | null               // FK to content folder (for organization)
...
enum LicenseType {
  OWNED = "OWNED",                       // Advertiser owns content
  LICENSED = "LICENSED",                 // Licensed from third party
  STOCK = "STOCK",                       // Stock photo/video
  USER_GENERATED = "USER_GENERATED"      // UGC with permissions
}
```
...
interface ContentVersion {
  ...
  asset_id: string                       // FK to content_asset
  version_number: number                 // 1, 2, 3, ...
  file_url: string                       // URL to this version
  change_description: string             // "Updated logo size"
...
interface ContentTag {
  tag_id: string                         // UUID
  advertiser_id: string                  // FK (tags are per advertiser)
  tag_name: string                       // "sale", "holiday", "new-product"
  usage_count: number                    // # of assets with this tag
  created_at: Date
}
```
