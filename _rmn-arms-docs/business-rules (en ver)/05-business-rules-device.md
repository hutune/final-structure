# Device Management Module - Business Rules Document
...
6. [Heartbeat & Health Monitoring](#heartbeat--health-monitoring)
7. [Content Synchronization](#content-synchronization)
8. [Playback Management](#playback-management)
9. [Device Configuration](#device-configuration)
...
### Purpose
This document defines ALL business rules for the Device Management module in RMN-Arms platform. Devices are the physical digital signage displays that show advertising content in retail stores.
...
#### 4.2 Heartbeat Payload Specification
```json
{
  "device_id": "uuid",
  "device_timestamp": "2026-01-23T10:05:00+07:00",
  "server_timestamp": null, // Filled by server
  ...
  "metrics": {
    ...
    "temperature_celsius": 42 // Optional, for hardware monitoring
  },
  "playback": {
    "screen_on": true,
    "content_playing": true,
    ...
    "playlist_id": "uuid",
    "playlist_position": 3, // Current position in playlist
    "playlist_total": 10, // Total items in playlist
  ...
  "errors": [
    {
      ...
      "message": "Failed to load content asset abc123",
...
✓ signature valid (signed with device private key)
✓ sequence_number > last_sequence_number (prevent replay attacks)
✓ device_timestamp within ±10 minutes of server time
✓ status is valid enum value
✓ metrics.cpu_usage_percent between 0-100
✓ metrics.memory_usage_percent between 0-100
✓ metrics.disk_usage_percent between 0-100
✓ All UUIDs (content_id, campaign_id) exist in database
```
...
```json
{
  ...
  "next_heartbeat_interval": 300, // Seconds
  ...
  "messages": [
    {
      "type": "INFO",
      "text": "System maintenance scheduled for 2026-01-24 02:00 AM"
    }
  ]
}
```
